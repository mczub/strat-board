...
const OG_BASE_URL = 'https://strat-board.mczub.workers.dev'
...
export const Route = createFileRoute('/c/$code')({
    ...
    head: ({ params }) => {
        ...
        try {
            const result = decode(code.startsWith('stgy:') ? `[${code}]` : code)
            if (result.name) {
                title = result.name
            }
...
function ViewBoardPage() {
    ...
    try {
        ...
        const fullCode = decodedCode.startsWith('stgy:') ? `[${decodedCode}]` : decodedCode
